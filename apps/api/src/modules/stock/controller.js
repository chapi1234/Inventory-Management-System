...
async function create(req, res, next) {
  try {
    ...
    created(res, result, 'Successfully created record in stock');
  } catch (err) {
    next(err);
  }
}
...
/**
 * Retrieve a paginated list of stock
 */
async function list(req, res, next) {
  try {
    ...
    if (search) {
      ...
      if ('stock' === 'products') filter.name = { $regex: search, $options: 'i' };
      if ('stock' === 'sales') filter.orderNumber = { $regex: search, $options: 'i' };
    }
...
/**
 * Delete a stock record completely
 */
