...
async function create(req, res, next) {
  try {
    ...
    created(res, result, 'Successfully created record in suppliers');
  } catch (err) {
    next(err);
  }
}
...
async function list(req, res, next) {
  try {
    ...
    if (search) {
      ...
      if ('suppliers' === 'products') filter.name = { $regex: search, $options: 'i' };
      if ('suppliers' === 'sales') filter.orderNumber = { $regex: search, $options: 'i' };
    }
...
/**
 * Retrieve a specific suppliers by ID
 */
...
/**
 * Update an existing suppliers record
 */
...
/**
 * Delete a suppliers record completely
 */
