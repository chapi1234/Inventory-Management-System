...
async function create(req, res, next) {
  try {
    ...
    created(res, result, 'Successfully created record in reports');
  } catch (err) {
    next(err);
  }
}
...
async function list(req, res, next) {
  try {
    ...
    if (search) {
      ...
      if ('reports' === 'products') filter.name = { $regex: search, $options: 'i' };
      if ('reports' === 'sales') filter.orderNumber = { $regex: search, $options: 'i' };
    }
...
/**
 * Retrieve a specific reports by ID
 */
...
/**
 * Update an existing reports record
 */
...
/**
 * Delete a reports record completely
 */
