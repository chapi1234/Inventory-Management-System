...
async function create(req, res, next) {
  try {
    ...
    created(res, result, 'Successfully created record in users');
  } catch (err) {
    next(err);
  }
}
...
async function list(req, res, next) {
  try {
    ...
    if (search) {
      ...
      if ('users' === 'products') filter.name = { $regex: search, $options: 'i' };
      if ('users' === 'sales') filter.orderNumber = { $regex: search, $options: 'i' };
    }
...
/**
 * Retrieve a specific users by ID
 */
...
/**
 * Delete a users record completely
 */
